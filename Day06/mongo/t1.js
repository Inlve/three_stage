...
getDataBase = () => {
    return new Promise(resolve => {
        connection().then(client => {
            resolve(client.db('admin').admin().listDatabases());
            client.close();
        });
    });
};
...
getCollection = (col) => {
    return new Promise((resolve, reject) => {
        connection().then(client => {
            client.db(col).listCollections({}, { nameOnly: true }).toArray((error, result) => {
                error ? reject(error) : resolve(result);
                client.close();
            })
        })
 ...
 * @returns {Promise}
 */
const update = ({ dbName = db, col, condition = {}, set, type = true }) => new Promise(
    async (resolve, reject) => {
        ...
        if (type) {
            ...
            client.db(dbName).collection(col).updateOne(condition, { $set: set }, (error, result) => {
                temp(error, result);
            })
        }
        client.close();
 ...
 * @returns {Promise}
 */
const delData = ({ dbName = db, col, condition = {}, type = true }) => new Promise(
    async (resolve, reject) => {
        function temp (error, result) {
            error ? reject(error) : resolve(result);
 ...
 * @returns {Promise}
 */
const find = ({ dbName = db, col, query = {}, sort = null, limit = 0, skip = 0 }) => new Promise(
    async (
        resolve, reject) => {
        const client = await connection();
        let temp = client.db(dbName).collection(col).find(query);
        if (!!sort) {
            temp = temp.sort(sort);
        }
        temp.skip(0).limit(0).toArray((error, result) => {
            error ? reject(error) : resolve(result);
        });
        client.close();
    });
